...
# class FeaturePairsDatasetLoFTR(torch.utils.data.Dataset):
#     def __init__(self, pairs, feature_path_q, feature_path_r):
#         self.pairs = pairs
#         self.feature_path_q = feature_path_q
#         self.feature_path_r = feature_path_r
...
#     print(f"Original size: {size}, scale: {scale}, new size: {size_new}")
#     image = resize_image(image, size_new, interp)
#     return image, np.mean([size_new[0]/size[0], size_new[1]/size[1]])
...
# def match_from_paths_glue_loftr(conf: Dict,
#                      pairs_path: Path,
#                      match_path: Path,
#                      feature_path_q: Path,
#                      feature_path_ref: Path,
...
#         # if len(keypts1.shape) == 3:
#         #     keypts1 = keypts1.squeeze()
#         # print(f"keypts0.shape: {keypts0.shape}, keypts1.shape: {keypts1.shape}") # 1, N, 2
#         # # show max of width in keypoints, max of height in keypoints, and number of keypoints
#         # max_width_kpt0 = torch.max(keypts0[:, :, 0])
#         # max_height_kpt0 = torch.max(keypts0[:, :, 1])
#         # num_kpt0 = keypts0.shape[1]
#         # max_width_kpt1 = torch.max(keypts1[:, :, 0])
#         # max_height_kpt1 = torch.max(keypts1[:, :, 1])
#         # num_kpt1 = keypts1.shape[1]
#         # print(f"max_width_kpt0: {max_width_kpt0}, max_height_kpt0: {max_height_kpt0}, num_kpt0: {num_kpt0}")
#         # print(f"max_width_kpt1: {max_width_kpt1}, max_height_kpt1: {max_height_kpt1}, num_kpt1: {num_kpt1}")
#         #
#         # keypts0 = data0['keypoints'][0].unsqueeze(0)
#         # keypts1 = data1['keypoints'][0].unsqueeze(0)
#         scores0 = data0['scores'][0].unsqueeze(0).to(device)
#         scores1 = data1['scores'][0].unsqueeze(0).to(device)
#         image_size0 = data0['image_size'].to(device)
#         # image_size0 = torch.tensor(image0.size).to(device)
#         image_size1 = data1['image_size'].to(device)
#         # image_size1 = torch.tensor(image1.size).to(device)
#         # if idx == 0:
#         #     print(f"image_size0: {image_size0}, image_size1: {image_size1}")
#         desc0 = data0['descriptors'].permute(0,2,1)
#         desc0 = desc0[0]
#         desc0 = desc0.unsqueeze(0).permute(0,2,1).to(device)
#         desc1 = data1['descriptors'].permute(0,2,1)
...
#     img2_cpy = img2.copy()
#     if len(img1.shape) == 2:
#         img1_cpy = cv2.cvtColor(img1_cpy, cv2.COLOR_GRAY2RGB)

#     if len(img2.shape) == 2:
#         img2_cpy = cv2.cvtColor(img2_cpy, cv2.COLOR_GRAY2RGB)

#     H1, W1 = img1_cpy.shape[:2]
#     H2, W2 = img2_cpy.shape[:2]
...
#     canvas[y1_min:y1_max, x1_min:x1_max] = img1_cpy[y1_min:y1_max, x1_min:x1_max]
#     cv2.rectangle(canvas, (x1_min, y1_min), (x1_max, y1_max), (0, 0, 255), 2)
#     cv2.putText(canvas, f"img1_cpy (id={id1})", (x1_min + 10, y1_min + 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)

#     # --- 3. DÁN ẢNH 2 VÀ VẼ KHUNG ---
#     # Tọa độ X trên canvas phải cộng thêm W1 (vì ảnh 2 nằm bên phải)
#     canvas[y2_min:y2_max, W1 + x2_min : W1 + x2_max] = img2_cpy[y2_min:y2_max, x2_min:x2_max]
#     cv2.rectangle(canvas, (W1 + x2_min, y2_min), (W1 + x2_max, y2_max), (0, 0, 255), 2)
#     cv2.putText(canvas, f"img2_cpy (id={id2})", (W1 + x2_min + 10, y2_min + 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
...
#             cv2.circle(canvas, (x2 + W1, y2), 1, (0, 0, 0), -1)
...
#     plt.imshow(canvas)
#     plt.title(f"Matches Visualization (img1_cpy_id: {id1}, img2_cpy_id: {id2})")
#     plt.axis('off')
#     plt.show()
...
#         # print(f"-----------pair path: {pairs_path}------------")
#         pairs = match_from_paths_glue_loftr(conf, pairs_path, match_path, feature_path_q, feature_path_ref, overwrite)
#     elif 'loftr' in conf['model']['name']:
#         pairs = match_from_paths_glue_loftr(conf, pairs_path, match_path, feature_path_q, feature_path_raw_ref, overwrite)
#     logger.info('Matching local features with configuration:'
#                 f'\n{pprint.pformat(conf)}')
#     if not feature_path_q.exists():
#         raise FileNotFoundError(f'Query feature file {feature_path_q}.')
...
#         dataset = FeaturePairsDatasetLoFTR(pairs, feature_path_q, feature_path_ref)
#     else:
#         dataset = FeaturePairsDataset(pairs, feature_path_q, feature_path_ref)
#         writer_queue = WorkQueue(partial(writer_fn, match_path=match_path), 5)
#     # print(f"dataset len: {dataset.__len__()}")
...
def main(conf: Dict,
    ...
    if features_ref is None:
        features_ref = features_q
    match_from_paths(conf, pairs, matches, features_q, features_ref, overwrite, dict_keypoints_index_query, dict_keypoints_index_map, is_query_map_match=is_query_map_match, feature_path_raw_ref=feature_path_raw_ref, method_baseline=method_baseline, method1=method1, method2=method2, method3=method3)
    # match_from_paths_dkm(conf, pairs, matches, features_q, features_ref, overwrite, dict_keypoints_index_query, dict_keypoints_index_map, is_query_map_match=is_query_map_match, feature_path_raw_ref=feature_path_raw_ref, method_baseline=method_baseline, method1=method1, method2=method2, method3=method3)
