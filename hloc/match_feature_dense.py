...
class FeaturePairsDatasetDense(torch.utils.data.Dataset):
...
def main(conf: Dict,
    ...
    match_from_paths(conf, pairs, matches, features_q, features_ref, overwrite, dict_keypoints_index_query,
                     dict_keypoints_index_map, is_query_map_match=is_query_map_match, feature_path_raw_ref=feature_path_raw_ref,
                     method_baseline=method_baseline, method1=method1, method2=method2, method3=method3, visualize=visualize)
    return matches
...
def match_from_paths_sparse(conf: Dict,
                     pairs_path: Path,
                     match_path: Path,
                     feature_path_q: Path,
    ...
    Model_lg = dynamic_load(matchers, conf['model_sparse']['name'])
    model_lg = Model_lg(conf['model_sparse']).eval().to(device)
    dataset = FeaturePairsDatasetDense(pairs, feature_path_q, feature_path_ref)
    ...
    for idx, data in enumerate(tqdm(loader, smoothing=.1)):            
        ...
        interp = 'cv2_area'
        resize_max = conf['model_sparse']['preprocessing']['resize_max']
        ...
        if len(data0['keypoints'].shape) == 4:
            keypts0 = data0['keypoints'][0]
        else:
            keypts0 = data0['keypoints']
        if len(data1['keypoints'].shape) == 4:
            keypts1 = data1['keypoints'][0]
        else:
            keypts1 = data1['keypoints']
    
        scores0 = data0['scores'][0].unsqueeze(0)
        scores1 = data1['scores'][0].unsqueeze(0)
        image_size0 = data0['image_size'].to(device)
        image_size1 = data1['image_size'].to(device)
        desc0 = data0['descriptors'].permute(0,2,1)
        desc0 = desc0[0]
        desc0 = desc0.unsqueeze(0).permute(0,2,1)
        desc1 = data1['descriptors'].permute(0,2,1)
        desc1 = desc1[0]
        desc1 = desc1.unsqueeze(0).permute(0,2,1)
...
def process_existed_kpts(kpts: torch.Tensor, patch_number: int, im_size):
    kpts_cpy = kpts.clone()
    if patch_number == 0:
        return kpts_cpy
    elif patch_number == 1:
        ...
        kpts_cpy*=2
        kpts_cpy -= kpts_cpy.new_tensor([im_size[0], im_size[1]])
    return kpts_cpy
...
def restore_coords(keypoints, patch_number, size, device):
    ...
    if patch_number == 0:
        ...
        keypoints = keypoints//2
        offset = keypoints.new_tensor([size[0]//2, size[1]//2])
        keypoints += offset
    return keypoints
...
def draw_custom_matches(img1, img2, kpts1, kpts2, matches, id1=0, id2=0):
    """
    draw matched keypoints between two images based on image patch id1 and image patch id2.
    """

    kpts1_int = kpts1.to(torch.int)
    kpts2_int = kpts2.to(torch.int)
    H1, W1 = img1.shape[:2]
    H2, W2 = img2.shape[:2]
    ...
    for i, pt1 in enumerate(kpts1_int):
        ...
        if x1_min <= x1 < x1_max and y1_min <= y1 < y1_max:
            ...
            for j in range(m_cols):
                idx2 = matches[i, j]
                if idx2 != -1:
                    x2, y2 = int(kpts2_int[idx2][0]), int(kpts2_int[idx2][1])
...
def match_from_paths(conf: Dict,
    ...
    pairs = []
    if "model_dense" not in conf.keys():
        print("this function is for dense matching only")
        return
    if not is_query_map_match:
        pairs = match_from_paths_sparse(conf, pairs_path, match_path, feature_path_q, feature_path_ref, overwrite)
    else:
        pairs = match_from_paths_sparse(conf, pairs_path, match_path, feature_path_q, feature_path_raw_ref, overwrite)
    logger.info('Matching local features with configuration:'
                f'\n{pprint.pformat(conf)}')
    if not feature_path_q.exists():
        raise FileNotFoundError(f'Query feature file {feature_path_q}.')
    ...
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    Model = dynamic_load(matchers, conf['model_dense']['name'])
    model = Model(conf['model_dense']).eval().to(device)
    dataset = FeaturePairsDatasetDense(pairs, feature_path_q, feature_path_ref)
    loader = torch.utils.data.DataLoader(
    ...
    for idx, data in enumerate(tqdm(loader, smoothing=.1)):
        ...
        kpt0, kpt1 = data0['keypoints'], data1['keypoints']
        kpt0_origin = kpt0.clone()
        kpt1_origin = kpt1.clone()
        # H, W, 3
        if len(kpt0.shape) > 3:
            kpt0 = kpt0.squeeze().unsqueeze(0) # 1, N, 2
        if len(kpt1.shape) > 3:
            kpt1 = kpt1.squeeze().unsqueeze(0) # 1, N, 2
        ...
        if method_baseline:
            ...
            with h5py.File(str(match_path), 'r', libver='latest') as fm:
                ...
                old_list_matches0 = list(fm[pair]['matches0'][:])
                old_list_matches0_origin = old_list_matches0.copy()
                old_list_matching_scores0 = list(fm[pair]['matching_scores0'][:])
            ...
            with h5py.File(str(match_path), 'a', libver='latest') as fm:
                del fm[pair]['matches0']
                del fm[pair]['matching_scores0']
                fm[pair].create_dataset('matches0', data=old_list_matches0_f)
                fm[pair].create_dataset('matching_scores0', data=old_list_matching_scores0_f)
            continue
        
        interp = 'cv2_area'
        resize_max = conf['model_dense']['resize_max']
        is_gray_scale = False
        if "gray_scale" in conf['model_dense'].keys():
            if conf['model_dense']["gray_scale"] == True:
                is_gray_scale = True
        image0, scales_0 = get_image(img_path0, interp, resize_max, is_gray_scale)
        h0, w0 = image0.shape[:2]
        kpt0 = kpt0 / scales_0
        image1, scales_1 = get_image(img_path1, interp, resize_max, is_gray_scale)
        ...
        for id0 in range(5):
            for id1 in range(5):
                ...
                data_input = None
                if "loftr" in conf['model_dense']['name']:
                    ...
                    data_input = [patch_img0, apatch_kpts0, patch_img1, apatch_kpts1, sub_patch_number]
                ...
                if not method_baseline:
                    ...
                    for j in range(len(index_nearest_2_kpts)):
                        if (index_nearest_2_kpts[j][0] == -1) or (index_nearest_2_kpts[j][1] == -1):
                            continue
                        keypoint_query = (int(kpt0[index_nearest_2_kpts[j][0]][0]*scales_0), int(kpt0[index_nearest_2_kpts[j][0]][1]*scales_0)) 
                        keypoint_reference = (int(kpt1[index_nearest_2_kpts[j][1]][0]*scales_1), int(kpt1[index_nearest_2_kpts[j][1]][1]*scales_1))
                        if str(keypoint_query) not in dict_index_keypoints_query:
                    ...
                    if method2:
                        ...
                        for j in range(score_map_less_query_larger.shape[0]):
                            x, y = keypoints_match0_map_less_query_larger[j][0], keypoints_match0_map_less_query_larger[j][1]
                            new_query_keypoint = (int(x*scales_0), int(y*scales_0))
                            old_map_keypoint = (int(keypoint_match1_map_less_sp[j][0] * scales_1), int(keypoint_match1_map_less_sp[j][1]*scales_1))
                            ...
                            if str(new_query_keypoint) not in dict_index_keypoints_query:
                                ...
                                new_query_keypoint = torch.tensor([x, y], device=kpt0.device, dtype=kpt0.dtype)
                                kpt0 = torch.cat([kpt0, new_query_keypoint.unsqueeze(0)], dim=0)

                        # process for map larger and query less, only if not is query_map
                        if not is_query_map_match:
                            for j in range(score_map_larger_query_less.shape[0]):
                                x, y = keypoint_match1_map_larger_query_less[j][0], keypoint_match1_map_larger_query_less[j][1]
                                new_map_keypoint = (int(x*scales_1), int(y*scales_1))
                                old_query_keypoint = (int(keypoints_match0_query_less_sp[j][0]*scales_0), int(keypoints_match0_query_less_sp[j][1]*scales_0))
                                ...
                                if str(new_map_keypoint) not in dict_index_keypoints_reference:
                                    ...
                                    new_map_keypoint = torch.tensor([x, y], device=kpt1.device, dtype=kpt1.dtype)
                                    kpt1 = torch.cat([kpt1, new_map_keypoint.unsqueeze(0)], dim=0)
                        ...
                        if not is_query_map_match:
                            for j in range(score_larger_than_threshold.shape[0]):
                                old_len_kpt1 = len(kpt1)
                                old_len_kpt0 = len(kpt0)
                                x_q, y_q = keypoints_match0_larger_than_threshold[j] ## different in float but same in int
                                new_keypoint_query = (int(x_q*scales_0), int(y_q*scales_0))
                                x_r, y_r = keypoint_match1_larger_than_threshold[j]
                                new_keypoint_reference = (int(x_r*scales_1), int(y_r*scales_1))
                                ...
                                new_keypoint_query = torch.tensor([x_q, y_q], device=kpt0.device, dtype=kpt0.dtype)
                                new_keypoint_reference = torch.tensor([x_r, y_r], device=kpt1.device, dtype=kpt1.dtype)
                                kpt0 = torch.cat([kpt0, new_keypoint_query.unsqueeze(0)], dim=0)
                                kpt1 = torch.cat([kpt1, new_keypoint_reference.unsqueeze(0)], dim=0)
                
                try:
                    # write back kpt0 and kpt1 to feature_path_q and feature_path_ref
                    kpt0 = kpt0*scales_0
                    kpt1 = kpt1*scales_1
                    kpt0 = kpt0.unsqueeze(0).cpu().numpy().astype(np.int32)
                    kpt1 = kpt1.unsqueeze(0).cpu().numpy().astype(np.int32)
                    ...
                    # reconvert kpt to resized
                    kpt0 = kpt0 / scales_0
                    kpt1 = kpt1 / scales_1
                    kpt0 = torch.from_numpy(kpt0).to(device)
                    kpt1 = torch.from_numpy(kpt1).to(device)
                    ...
                    kpt0 = kpt0_origin*scales_0
                    kpt1 = kpt1_origin*scales_1
                    kpt0 = kpt0_origin.unsqueeze(0).cpu().numpy().astype(np.int32)
                    kpt1 = kpt1_origin.unsqueeze(0).cpu().numpy().astype(np.int32)
                    with h5py.File(str(feature_path_q), 'a', libver='latest') as fq:
                        uncertainty = 2.0*scales_0
                        del fq[name_img0]['keypoints']
                        fq[name_img0].create_dataset('keypoints', data=kpt0)
                    ...
                    with h5py.File(str(match_path), 'a', libver='latest') as fm:
                        ...
                        del fm[pair]['matches0']
                        del fm[pair]['matching_scores0']
                        fm[pair].create_dataset('matches0', data=old_list_matches0_origin)
                        fm[pair].create_dataset('matching_scores0', data=old_list_matching_scores0_origin)
                ...
                if visualize:
                    if "gray_scale" in conf["model_dense"].keys() and conf["model_dense"]["gray_scale"] == True:
